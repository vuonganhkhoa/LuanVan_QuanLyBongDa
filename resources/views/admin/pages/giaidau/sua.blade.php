								...
								<input type="hidden" name="_token" value="{{ csrf_token() }}">
									...
									<div class="col-md-12">
										<div class="form-group {{ $errors->has('tengiaidau') ? 'has-error' : '' }}">
											...
											<div class="col-md-3">
												<input style="height: 30px; width: 200px" class="form-control1" type="text" name="tengiaidau" value="{{ $giaidau->TenGiaiDau }}">
											</div>
									...
									<div class="col-md-12">
										<div class="form-group {{ $errors->has('nambatdau') ? 'has-error' : '' }}">
											...
											<div class="col-md-3">
												<input style="height: 30px; width: 200px" type="date" class="form-control date"  name="nambatdau" value="{{$giaidau->NamBatDauMuaGiai}}">
											</div>
									...
									<div class="col-md-12">
										<div class="form-group {{ $errors->has('namketthuc') ? 'has-error' : '' }}">
											...
											<div class="col-md-3">
												<input style="height: 30px; width: 200px" type="date" class="form-control date"  name="namketthuc" value="{{$giaidau->NamKetThucMuaGiai}}">
											</div>
										...
										<div class="form-group {{ $errors->has('muagiaihientai') ? 'has-error' : '' }}">
											...
											<div class="col-md-3">
												<select style="width: 200px;" class="control-label " name="muagiaihientai" >
					                              <option value="0"
					                              		@if($giaidau->MuaGiaiHienTai == 0)
					                              			{{'Selected'}}
					                              		@endif
					                              >Mùa giải khác</option>

					                              <option value="1"
					                              		@if($giaidau->MuaGiaiHienTai == 1)
					                              			{{'Selected'}}
					                              		@endif
					                              >Mùa giải hiện tại</option>
					                            </select>
											</div>
